...
public:
 ...
 void deleteatidx(int idx){  // delete ele at any specific posn
     if(size==0){
        cout<<"list is empty";
    }
    ...
    else if(idx==size-1)return deleteatatil();
    else{
        node*temp=head;
        for(int i=1;i<=idx-1;i++){  // its for going on the idx which we need
                temp=temp->next;
            }
